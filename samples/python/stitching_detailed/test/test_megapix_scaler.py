...
from stitching_detailed.downscaler import Downscaler
#%%
...
class TestImageData(unittest.TestCase):
    ...
    def test_if_scaler_is_set(self):
        ...
        scaler = ImageToMegapixScaler(0.6)
        scale_img1 = scaler.get_scale_by_image(img1)
        scale_img2 = scaler.get_scale_by_image(img2)
        scaler.set_scale_if_not_set(scale_img1)
        scaler.set_scale_if_not_set(scale_img2)

        scaler_scale = scaler.scale

        self.assertIsNot(scaler_scale, scale_img2)

    def test_estimate_original_img_size(self):
        img1, img2 = cv.imread("boat1.jpg"), cv.imread("boat2.jpg")
        scaler = ImageToMegapixScaler(0.6)
        scale = scaler.get_scale_by_image(img1)
        scaler.set_scale_if_not_set(scale)
        resized = scaler.resize(img2)

        original = scaler.estimate_original_img_size(resized)

        self.assertEqual(original, [3889, 2590])
    ...
    def test_force_of_downscaling(self):
        img1 = cv.imread("s1.jpg")
        normal_scaler = ImageToMegapixScaler(2)
        downscaler = Downscaler(2)

        normal_scale = normal_scaler.get_scale_by_image(img1)
        downscale_scale = downscaler.get_scale_by_image(img1)
